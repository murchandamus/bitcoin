...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, std::chrono::microseconds current_time)
{
    ...
    CAmount currentFilter = m_mempool.GetMinFee(gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000).GetFeeRate();
    ...
    if (current_time > pto.m_tx_relay->m_next_send_feefilter) {
        ...
        filterToSend = std::max(filterToSend, ::minRelayTxFee.GetFeeRate());
