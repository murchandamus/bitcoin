...
util::Result<SelectionResult> OldestFirst(std::vector<OutputGroup>& utxo_pool, const CAmount& selection_target, CAmount change_target, int max_weight)
{
    SelectionResult result(selection_target, SelectionAlgorithm::OF);
    std::priority_queue<OutputGroup, std::vector<OutputGroup>, MinOutputGroupComparator> heap;

    // Include a minimum change budget for Oldest First as we want to avoid
...
std::string GetAlgorithmName(const SelectionAlgorithm algo)
{
    ...
    case SelectionAlgorithm::OF: return "of";
