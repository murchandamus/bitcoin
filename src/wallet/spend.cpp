...
util::Result<SelectionResult> ChooseSelectionResult(interfaces::Chain& chain, const CAmount& nTargetValue, Groups& groups, const CoinSelectionParams& coin_selection_params)
{
    ...
    // Allow Youngest First to spend UTXOs with negative effective value at any feerate
    if (auto yf_result{YoungestFirst(groups.mixed_group, nTargetValue, coin_selection_params.m_min_change_target, max_inputs_weight)}) {
        results.push_back(*yf_result);
    } else append_error(yf_result);
