        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    TRACE5(coin_selection, selected_coins, wallet.GetDisplayName().c_str(), GetAlgorithmName(result->m_algo).c_str(), result->m_target, result->GetWaste(), result->GetSelectedValue());
        ...
        bool sign)
{
    ...
    TRACE4(coin_selection, normal_create_tx_internal, wallet.GetDisplayName().c_str(), res, nFeeRet, nChangePosInOut);
    // try with avoidpartialspends unless it's enabled already
    if (res && nFeeRet > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        TRACE1(coin_selection, attempting_aps_create_tx, wallet.GetDisplayName().c_str());
        ...
        if (CreateTransactionInternal(wallet, vecSend, tx2, nFeeRet2, nChangePosInOut2, error2, tmp_cc, fee_calc_out, sign)) {
            ...
            TRACE5(coin_selection, aps_create_tx_internal, wallet.GetDisplayName().c_str(), use_aps, res, nFeeRet2, nChangePosInOut2);
            if (use_aps) {
                tx = tx2;
                nFeeRet = nFeeRet2;
