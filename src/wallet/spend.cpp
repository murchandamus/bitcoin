                               ...
                               const CoinSelectionParams& coin_selection_params)
{
    ...
    const CAmount target_with_change = nTargetValue + coin_selection_params.m_change_fee + MIN_FINAL_CHANGE;
    if (auto srd_result{SelectCoinsSRD(positive_groups, target_with_change)}) {
        srd_result->ComputeAndSetWaste(coin_selection_params.m_cost_of_change);
        results.push_back(*srd_result);
    }

    if (auto fifo_result{SelectCoinsFIFO(positive_groups, target_with_change)}) {
