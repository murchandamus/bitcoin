...
util::Result<SelectionResult> ChooseSelectionResult(interfaces::Chain& chain, const CAmount& nTargetValue, Groups& groups, const CoinSelectionParams& coin_selection_params)
{
    ...
    // Allow Oldest First to spend UTXOs with negative effective value at any feerate
    if (auto of_result{OldestFirst(groups.mixed_group, nTargetValue, coin_selection_params.m_min_change_target, max_inputs_weight)}) {
        results.push_back(*of_result);
    } else append_error(of_result);
