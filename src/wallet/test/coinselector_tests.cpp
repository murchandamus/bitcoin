...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            std::cout << "ALTERNATIVE #5" << std::endl;
            for (int j = 0; j < 11; ++j) {
                // Add heavy coins {1, 2, 3, …, 11}
                add_coin(available_coins, wallet, CAmount((1 + 1* j) * COIN), CFeeRate(5000), 144, false, 0, true, 350);
                // Add medium coins {0.9, 1.9, 2.9, …, 10.9}
                add_coin(available_coins, wallet, CAmount((0.9 + 1 * j) * COIN), CFeeRate(5000), 144, false, 0, true, 250);
                // Add light coins {0.8, 1.8, 2.8, …, 10.8}
                add_coin(available_coins, wallet, CAmount((0.8 + 1 * j) * COIN), CFeeRate(5000), 144, false, 0, true, 150);
            }
            return available_coins;
        });
        BOOST_CHECK(res);
        // If this takes more attempts, the implementation has regressed
        size_t expected_attempts = 505;
        BOOST_CHECK_MESSAGE(res->GetSelectionsEvaluated() == expected_attempts, strprintf("Expected %i attempts, but got %i", expected_attempts, res->GetSelectionsEvaluated()));
    }
