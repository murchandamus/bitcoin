...
static util::Result<SelectionResult> YoungestFirstHelper(const CAmount& target,
                                                 const CoinSelectionParams& cs_params,
                                                 const node::NodeContext& m_node,
                                                 int max_weight,
                                                 std::function<CoinsResult(CWallet&)> coin_setup)
{
    ...
    return YoungestFirst(groups.mixed_group, target, cs_params.m_min_change_target, max_weight);
}

BOOST_AUTO_TEST_CASE(youngest_first_tests)
{
    // Test YoungestFirst:
...

    {
        ...
        CAmount target = 49.5L * COIN;
        int max_weight = 10000; // high enough to not fail for this reason.
        const auto& res = YoungestFirstHelper(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                add_coin(available_coins, wallet, CAmount(1 * COIN));
                add_coin(available_coins, wallet, CAmount(2 * COIN));
