...
class CoinSelectionSimulation(BitcoinTestFramework):
    ...
    def log_sim_results(self, res_file):
        ...
        result_str = f"| {self.scenario_name} | {self.tester.getbalance()} | {mean_utxo_set_size} | {len(remaining_utxos)} | {self.count_received} | {self.count_sent} | {self.withdraws} | {self.unec_utxos} | {len(self.change_vals)} | {no_change_str} | {min_change} | {max_change} | {mean_change} | {stdev_change} | {self.total_fees} | {mean_fees} | {cost_to_empty} | {total_cost} | {min_input_size} | {max_input_size} | {mean_input_size} | {stdev_input_size} | {usage_str} |"
    ...
    def run_test(self):
        ...
        self.scenario_name = os.path.basename(self.scenario_path)
        ...
        with \
            ...
            open(self.scenario_path, "r") as scenario_data:
